...
export default function StepInterests() {
  ...
  return (
    <OnboardingStep 
      ...
      onNext={() => router.push({ pathname: "/(root)/(onboarding)/photos", params: { ...params, interests: selected.join(',') } })}
    >
      {loading ? (
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <View className="flex-row flex-wrap gap-2 pb-10 justify-center">
            {all.map(item => (
              ...
              >
                <Text className={`font-bold ${selected.includes(item.interest_id) ? 'text-white' : 'text-gray-400'}`}>{item.interest_name}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </ScrollView>
